(function($) {
  ...
  $.fn.treeMultiselect = function(opts) {
    ...
    this.each(function() {
      ...
      if(!opts.justOneTree) {
        $originalSelect =$(".selected2");
        $selectionContainer = $(".selected").find(".selections");
      }
      ...
      var $selectedContainer = $(uiBuilder.selected);
        ...
        $selectedContainer = $(".selected").find(".selections");
      }
...
//处理参数
  ...
  function updateSelectedAndOnChange($selectionContainer, $selectedContainer, $originalSelect, options) {
    ...
    function createSelectedDiv(selection) {
      ...
      var aa = $selectedContainer.find("div.item[data-value='"+value+"']");
      console.log(aa);
      $(item).attr('data-value', value)
        .appendTo(aa.parent());

      var path = sectionName.split(options.sectionDelimiter);
      var optionDescription ="";
      var optionIndex = selection.initialIndex;
    }
  ...
  // 初始化的时候建立三个div，在页面上的select后面建立大div里面有两个小div一左一右
